...
'''
The attention mechanism was applied to both the frequency domain features and the time domain features, 
and the frequency domain features were only expanded to 128.
'''
class Net1DWithAttention(nn.Module):
    def __init__(self, in_channels, base_filters, ratio, filter_list, m_blocks_list, kernel_size, stride, groups_width, n_classes, use_bn=True, use_do=True, verbose=False):
        super(Net1DWithAttention, self).__init__()
        ...
        # First conv layer for time-domain signal
        self.first_conv = MyConv1dPadSame(
            in_channels=in_channels, 
            out_channels=self.base_filters, 
            kernel_size=self.kernel_size, 
        ...
        # Fully connected layer for frequency-domain features without dropout
        ...
        # Reduction layer for time-domain features without dropout
        self.time_fc = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU()
        )

        # Final prediction layer without dropout
        self.final_fc = nn.Sequential(
            nn.Linear(1024 + 128, n_classes)
        )
    ...
    def forward(self, x, freq):
        ...
        freq_out = self.freq_fc(freq)

        # Attention mechanisms
        time_attn_out = self.time_attention(out)
        freq_attn_out = self.freq_attention(freq_out)

        # Reduce time-domain feature dimension
        time_attn_out = self.time_fc(time_attn_out)

        # Concatenate time-domain and frequency-domain features
        combined = torch.cat((time_attn_out, freq_attn_out), dim=1)
